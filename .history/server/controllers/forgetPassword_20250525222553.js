...
const forgetPassword = async (req, res, next) => {
    ...
    try {
        ...
        if (findedUser.otp.otp && new Date(findedUser.otp.sendTime).getTime() && findedUser.otp.sendTime > Date.now() - 5 * 60 * 1000) {
            const error = new Error('OTP already sent. Please wait before requesting a new one.');
