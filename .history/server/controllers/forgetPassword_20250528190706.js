...
const forgetPassword = async (req, res, next) => {
    ...
    try {
        ...
        if (findedUser.otp.otp && new Date(findedUser.otp.sendTime).getTime() > new Date().getTime()) {
            const waitTimeInMs = new Date(findedUser.otp.sendTime).getTime() - new Date().getTime();
            const waitTimeInSec = Math.ceil(waitTimeInMs / 1000);
            const error = new Error(`OTP already sent. Please wait ${waitTimeInSec} seconds before requesting again.`);
        ...
        console.log(`Send Time : ${findedUser.otp.sendTime}`);
