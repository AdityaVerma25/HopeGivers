...
const VerifyOtp = () => {
    ...
    const getTime = async () => {
        try {
            ...
            if (!response.ok) {
                // Show backend error message in toast and stop timer setup
                toast.error(result.message || 'Failed to fetch OTP time');
                setOtpTime(null); // Clear timer
                return;
            }
                ...
                <form onSubmit={submitHandler}>
                    <div className="auth_container">
                        ...
                        <div className='timer_container'>
                            {otpTime !== null && otpTime > 0 ? (
                                <Timer time={otpTime} />
                            ) : (
                                <p className="text-red-500 mt-2">OTP expired. Please request a new one.</p>
                            )}
                        </div>
